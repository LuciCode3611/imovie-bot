...
async def dashboard_action(
    ...
    if action == "open":
        stats = await _gather_stats(zarfilm, cfg, db, subdl)
        await _send_view(
            bot,
            callback.message,
...
async def dashboard(message: Message, bot: Bot, cfg: Config, zarfilm: ZarfilmClient, db: Database, subdl: SubdlClient | None = None) -> None:
    if not _is_owner(message.from_user, cfg):
        return
    stats = await _gather_stats(zarfilm, cfg, db, subdl)
...
async def _gather_stats(zarfilm: ZarfilmClient, cfg: Config, db: Database | None = None, subdl: SubdlClient | None = None) -> dict:
    ...
    stats = {
        ...
        "proxy": cfg.proxy_url,
        # SubDL counters are in-process only (no persisted aggregate yet)
        "sub_enabled": subdl.enabled if subdl is not None else False,
        "sub_searches": subdl.stats.get("searches", 0) if subdl is not None else 0,
        "sub_titles": subdl.stats.get("titles", 0) if subdl is not None else 0,
        "sub_downloads": subdl.stats.get("downloads", 0) if subdl is not None else 0,
    }
    if db is not None:
